...
imgproc::imgproc(QString Path, QString NoiseLevels, QString MedianFilters, QString LineRemoval, QString ColumnRemoval, QString RotAngles, QString XStretchFactors, QString YShearFactors, QString ScaleFactors, QString CropFactors, QString BandingFrequency, QString SaltAndPepper, QString GaussianNoise, QString SubstrateFile, bool eightBitConversion, QString EvaluationMode, QString PreProcessingMode, QString ShiftFactor, QString TiltParameter, bool enableBaseline, QString PassportScale, QString HaarCascadeFile, QString DoubleScale) {
    this->path=Path;
...
cv::Mat imgproc::removeBlackPixels(Mat input, Scalar mean) {
    if (input.channels()==3) {
        ...
        for (Mat3b::iterator it=temp.begin(); it!=temp.end(); it++)
            if (*it == Vec3b(0,0,0)) *it=Vec3b(mean[0], mean[1], mean[2]);
    }
    if (input.channels()==1) {
...
void imgproc::evaluateFile(QString filename, QString filepath) {
    ...
    if (this->NoiseLevels.length()>0) {
        ...
        for(int i=0; i<noiseInput.length();i++) additiveNoiseLevels << noiseInput[i].toInt();
        int noiseLevel;
        for (int i=0; i<additiveNoiseLevels.count(); i++) {
            ...
            if (DoEmbed) temp=embedOnSubstrate(temp.clone(), median);
            QDir dir=QDir(filepath);
            dir.mkpath(filepath+outputPath+"/additiveNoise/"+QString::number(noiseLevel)+"/");
            QString outputname=filepath+outputPath+"/additiveNoise/"+QString::number(noiseLevel)+"/"+baseName+"_additiveNoise_"+QString::number(noiseLevel);
    ...
    if (this->GaussianNoiseLevels.length()>0) {
        ...
        for(int i=0; i<GaussianNoiseInput.length();i++) additiveGaussianNoiseLevels << GaussianNoiseInput[i].toInt();
        ...
        for (int i=0; i<additiveGaussianNoiseLevels.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/additiveGaussianNoise/"+QString::number(GaussianNoiseLevel)+"/"+baseName+"_additiveGaussianNoise_"+QString::number(GaussianNoiseLevel);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->MedianFilters.length()>0) {
        ...
        for(int i=0; i<medianInput.length();i++) medianKernels << medianInput[i].toInt();
        int medianKernel;
        for (int i=0; i<medianKernels.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/medianCut/"+QString::number(medianKernel)+"/"+baseName+"_medianCut_"+QString::number(medianKernel);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->LineRemoval.length()>0) {
        ...
        for(int i=0; i<drowInput.length();i++) delRows << drowInput[i].toInt();
        int delRow;
        for (int i=0; i<delRows.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/removeLines/"+QString::number(delRow)+"/"+baseName+"_removeLines_"+QString::number(delRow);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->ColumnRemoval.length()>0) {
        ...
        for(int i=0; i<dcolInput.length();i++) delCols << dcolInput[i].toInt();
        int delCol;
        for (int i=0; i<delCols.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/removeColumns/"+QString::number(delCol)+"/"+baseName+"_removeColumns_"+QString::number(delCol);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->RotAngles.length()>0) {
        ...
        for(int i=0; i<rotInput.length();i++) rotAngles << rotInput[i].toDouble();
        double rotAngle;
        for (int i=0; i<rotAngles.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/rotation/"+QString::number(rotAngle)+"/"+baseName+"_rotation_"+QString::number(rotAngle);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->XStretchFactors.length()>0) {
        ...
        for(int i=0; i<xstretchInput.length();i++) xStretchFactors << xstretchInput[i].toDouble();
        ...
        for (int i=0; i<xStretchFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/XStretching/"+QString::number(xStretchFactor)+"/"+baseName+"_xStretch_"+QString::number(xStretchFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->YShearFactors.length()>0) {
        ...
        for(int i=0; i<yshearInput.length();i++) yShearFactors << yshearInput[i].toDouble();
        double yShearFactor;
        for (int i=0; i<yShearFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/yShearing/"+QString::number(yShearFactor)+"/"+baseName+"_yShear_"+QString::number(yShearFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->ScaleFactors.length()>0) {
        ...
        for(int i=0; i<scaleInput.length();i++) scaleFactors << scaleInput[i].toDouble();
        double scaleFactor;
        for (int i=0; i<scaleFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/rescale/"+QString::number(scaleFactor)+"/"+baseName+"_rescale_"+QString::number(scaleFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->CropFactors.length()>0) {
        ...
        for(int i=0; i<cropInput.length();i++) cropFactors << cropInput[i].toDouble();
        double cropFactor;
        for (int i=0; i<cropFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/cropping/"+QString::number(cropFactor)+"/"+baseName+"_cropping_"+QString::number(cropFactor);
            eval.evaluate(temp,scale,outputname);;
        }
    }
    ...
    if (this->BandingFrequency.length()>0) {
        ...
        for(int i=0; i<bandInput.length();i++) bandingFactors << bandInput[i].toDouble();
        ...
        for (int i=0; i<bandingFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/banding/"+QString::number(bandingFactor)+"/"+baseName+"_banding_"+QString::number(bandingFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->SaltAndPepperFactor.length()>0) {
        ...
        for(int i=0; i<SaltAndPepperInput.length();i++) SaltAndPepperFactors << SaltAndPepperInput[i].toInt();
        ...
        for (int i=0; i<SaltAndPepperFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/SaltAndPepper/"+QString::number(saltAndPepperFactor)+"/"+baseName+"_SaltAndPepper_"+QString::number(saltAndPepperFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->ShiftParameters.length()>0) {
        ...
        for(int i=0; i<ShiftInput.length();i++) ShiftFactors << ShiftInput[i].toDouble();
        double shiftFactor;
        for (int i=0; i<ShiftFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/shift/"+QString::number(shiftFactor)+"/"+baseName+"_shift_"+QString::number(shiftFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->TiltParameters.length()>0) {
        ...
        for(int i=0; i<TiltInput.length();i++) {
            ...
            if (DoEmbed) temp=embedOnSubstrate(temp.clone(), median);
            QDir dir=QDir(filepath);
            dir.mkpath(filepath+outputPath+"/tilt/("+QString::number(A)+","+QString::number(B)+","+QString::number(C)+")/");
            QString outputname=filepath+outputPath+"/tilt/("+QString::number(A)+","+QString::number(B)+","+QString::number(C)+")/"+baseName+"_tilt_("+QString::number(A)+","+QString::number(B)+","+QString::number(C)+")";
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->DouleScaleParameters.length()>0) {
        ...
        for(int i=0; i<DoubleScaleInput.length();i++) DoubleScaleFactors << DoubleScaleInput[i].toDouble();
        ...
        for (int i=0; i<DoubleScaleFactors.count(); i++) {
            ...
            QString outputname=filepath+outputPath+"/DoubleScale/"+QString::number(DoubleScaleFactor)+"/"+baseName+"_DoubleScale_"+QString::number(DoubleScaleFactor);
            eval.evaluate(temp,scale,outputname);
        }
    }
    ...
    if (this->PassportScaleParameters.length()>0) {
        ...
        for (int i=0 ; i<PassportScaleParameterWidth.count(); i++) {
            ...
            QDir dir=QDir(filepath);
            dir.mkpath(filepath+outputPath+"/PassportScale/"+QString::number(PSwidth)+"x"+QString::number(PSheight)+"/");
            QString outputname=filepath+outputPath+"/PassportScale/"+QString::number(PSwidth)+"x"+QString::number(PSheight)+"/"+baseName+"_PassportScale_"+QString::number(PSwidth)+"x"+QString::number(PSheight);
            eval.evaluate(temp,scale,outputname);
        }
    }
  }
...
void imgproc::processMatCombined(QString filename, QString filepath) {
    ...
    if (this->GaussianNoiseLevels.length()>0) {
        QStringList GaussianNoiseInput=this->GaussianNoiseLevels.split(",");
        for(int i=0; i<GaussianNoiseInput.length();i++) additiveGaussianNoiseLevels << GaussianNoiseInput[i].toInt();
    } else additiveGaussianNoiseLevels << 0;
    ...
    if (this->PassportScaleParameters.length()>0) {
      ...
      } else PassportScaleFactors << Vec2i(0,0);
                                    ...
                                    for (int scle=0; scle<scaleFactors.length(); scle++) {
                                        ...
                                        for (int crp=0; crp<cropFactors.length(); crp++) {
                                            ...
                                            for (int bnd=0; bnd<bandingFactors.length(); bnd++) {
                                                cv::Mat tempB=banding(50, (double)bandingFactors[bnd], tempR.clone(), median);
                                                QString nameTagB=nameTagR+"_bnd_"+QString::number((int)bandingFactors[bnd]);
                                                if (DoEmbed) tempB=embedOnSubstrate(tempB.clone(), median);
                                                for (int snp=0; snp<SaltAndPepperFactors.length(); snp++) {
                                                    cv::Mat tempP=SaltAndPepperNoise((int)SaltAndPepperFactors[snp], tempB.clone());
                                                    QString nameTagP=nameTagB+"_snp_"+QString::number((int)SaltAndPepperFactors[snp]);
                                                    for (int rotA=0; rotA<rotAngles.length(); rotA++) {
                                                        cv::Mat tempO=rotation((double)rotAngles[rotA], tempP.clone());
                                                        QString nameTagRot=nameTagP+"_rot_"+QString::number((int)rotAngles[rotA]);
                                                        for (int shiftV=0 ; shiftV < ShiftFactors.length() ; shiftV++) {
                                                            cv::Mat tempSH=shiftValues(tempO.clone(),ShiftFactors[shiftV]);
                                                            QString nameTagSH=nameTagRot+"_shift_"+QString::number((int)ShiftFactors[shiftV]);
                                                            for (int tiltV=0 ; tiltV < TiltFactors.length() ; tiltV++) {
                                                                cv::Mat tempTLT=tilt(TiltFactors[tiltV][0],TiltFactors[tiltV][1],TiltFactors[tiltV][2],tempSH.clone());
                                                                QString nameTagTLT=nameTagSH+"_tilt_"+QString::number((int)TiltFactors[tiltV][0])+"#"+QString::number((int)TiltFactors[tiltV][1])+"#"+QString::number((int)TiltFactors[tiltV][2]);
                                                                for (int dscaleI=0; dscaleI<DoubleScaleFactors.length(); dscaleI++) {
                                                                    cv::Mat tempDS=DoubleScale(DoubleScaleFactors[dscaleI], tempTLT.clone());
                                                                    QString nameTagDS=nameTagTLT+"_doublescale_"+QString::number((int)DoubleScaleFactors[dscaleI]);
                                                                    for (int PPSV=0 ; PPSV < PassportScaleFactors.length() ; PPSV++) {
                                                                        cv::Mat tempPPS=passportScale(PassportScaleFactors[PPSV][0],PassportScaleFactors[PPSV][1],tempDS.clone());
                                                                        QString nameTagPPS=nameTagDS+"_passportScale_"+QString::number((int)PassportScaleFactors[PPSV][0])+"x"+QString::number((int)PassportScaleFactors[PPSV][1]);

                                                                        QString FinalNameTag=nameTagPPS;
                                                                        cv::Mat finalMat = tempPPS;

                                                                        if (DoEmbed) FinalNameTag+="_"+this->EmbedFileName;
                                                                        dir.mkpath(filepath+outputPath+"/combined/"+FinalNameTag+"/");
                                                                        QString outputname=filepath+outputPath+"/combined/"+FinalNameTag+"/"+baseName+"_"+FinalNameTag;
...
*/
        ...
        if ((noisepattern.type()==input.type())&&(noisepattern.size()==input.size())) {
            ...
            if (noisepattern.type()!=input.type()) {qDebug() << "type missmatch" << endl;}
            if (noisepattern.size()!=input.size()) {qDebug() << "size missmatch" << endl;}
        }
    }
    catch (int e)
    {
...
cv::Mat imgproc::medianCutFiltering(int kernelSize, cv::Mat input) {
    if (kernelSize==0) return input;
    try {
        if (kernelSize%2==0) throw 5;
        if (input.depth()==0) cv::medianBlur(input,input,kernelSize);
            else {
            if (kernelSize<=5) cv::medianBlur(input,input,kernelSize);
            else throw 6;
            }
    }
    catch (int e)
...
cv::Mat imgproc::rotation (double angle, cv::Mat input){
    if (angle==0) return input;
    try {
        Point2f center=Point2f(input.cols/2, input.rows/2);
        cv::Mat rotA=cv::getRotationMatrix2D(center, angle, 1.0);//getRotationMatrix2D(rectA.center,angle_correction*(rectA.angle-rectB.angle), 1.0);
        cv::Mat output;
...
cv::Mat imgproc::rescale(double scalingfactor, cv::Mat input){
    if (scalingfactor==0) return input;
    try {
...
cv::Mat imgproc::cropping(double croppercentage, cv::Mat input){
    if (croppercentage==0) return input;
    try {
...
cv::Mat imgproc::banding(int width, double param, cv::Mat input, cv::Scalar mean){
    ...
    for (int i=0 ; i<input.rows; i++) {
        ...
        if ((((double)rand())/(double)RAND_MAX)>=1.0-(param)) {
            Mat bandingArtifact;
            if (i+width<input.rows) bandingArtifact=Mat(input, Rect(0,i,input.cols, width));
            else bandingArtifact=Mat(input, Rect(0,i,input.cols, input.rows-i));
...
short imgproc::SaltAndPepperHelper(int noiseLevel) {
    ...
    if (fabs(randVal)<0.5-threshold) return 0;
        else {
            if (randVal<0) return -1;
            if (randVal>0) return 1;
    }
    return 0;
}
            ...
            switch (bitdepth) {
                        case 0: //8bit unsigned
                                {
                                    ...
                                    for (it=temp1.begin(); it!=temp1.end(); it++) {
                                        ...
                                        if (helperRes!=0) {
                                            if (helperRes<0) *it=(quint8)min;
                                            if (helperRes>0) *it=(quint8)max;
                                            }
                                    }
                                    break;
                                    }
                        case 2: //16bit unsigned
                                    ...
                                    for (it=temp2.begin(); it!=temp2.end(); it++) {
                                        helperRes = SaltAndPepperHelper(noiseLevel);
                                        if (helperRes!=0) {
                                            if (helperRes<0) *it=(quint16)min;
                                            if (helperRes>0) *it=(quint16)max;
                                            }
                                    }
                                    break;
                                    }
                        case 4: //32bit unsigned
                                    ...
                                    for (it=temp3.begin(); it!=temp3.end(); it++) {
                                        helperRes = SaltAndPepperHelper(noiseLevel);
                                        if (helperRes!=0) {
                                            if (helperRes<0) *it=(quint32)min;
                                            if (helperRes>0) *it=(quint32)max;
                                            }
                                    }
                                    break;
                                    }
                        case 5: //32bit float
                                ...
                                for (it=temp4.begin(); it!=temp4.end(); it++) {
                                    helperRes = SaltAndPepperHelper(noiseLevel);
                                    if (helperRes!=0) {
                                        if (helperRes<0) *it=(float)min;
                                        if (helperRes>0) *it=(float)max;
                                        }
                                }
                                break;
                                }
...
cv::Mat imgproc::embedOnSubstrate(Mat trace, Scalar median) {
    if (this->Substrate.size == trace.size) {
        ...
        if ((this->Substrate.cols > trace.cols)&&(this->Substrate.rows > trace.rows)) {
            cv::Mat section=cv::Mat(this->Substrate, cv::Rect((this->Substrate.cols-trace.cols)/2, (this->Substrate.rows-trace.rows)/2, trace.cols, trace.rows));
            if (trace.type()!=this->Substrate.type()) {
                cv::Mat tmp;
            ...
            cv::Mat section=cv::Mat(trace, cv::Rect((trace.cols-this->Substrate.cols)/2, (trace.rows-this->Substrate.rows)/2, this->Substrate.cols, this->Substrate.rows));
            if (trace.type()!=this->Substrate.type()) {
                cv::Mat tmp,tmp2;
                this->Substrate.convertTo(tmp, trace.type());
                tmp2=section+tmp;
    ...
    try {
        cv::Mat output=Mat(targetHeight, targetWidth, input.type());
            ...
            if (Faces.size()==0) {
                ...
                if (cropwidth>input.cols) cropwidth=input.cols;
                if (cropheight>input.rows) cropheight=input.rows;
                cv::Mat roi=Mat(input, cv::Rect((input.cols-cropwidth)/2, (input.rows-cropheight)/2, cropwidth, cropheight));
                Mat tempf=roi.clone();
            ...
            int TLX=FcenterX-(cropwidth/2); if(TLX<0) TLX=0; if (TLX+cropwidth>input.cols) TLX=input.cols-cropwidth-1;
            int TLY=FcenterY-(cropheight/2); if (TLY<0) TLY=0; if (TLY+cropheight>input.rows) TLY=input.rows-cropheight-1;
