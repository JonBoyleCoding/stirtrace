...
void DDRawWriter::writeRawData(cv::Mat img, QString filename, double scale, double time) {
    ...
    if (scale>0) {
        ...
        if (metadataFile.open(QFile::ReadWrite)) {
                    ...
                    metadataStream << "AreaWidth="<<QString::number((double)img.cols/scale*1000000.0,'f')<< "\r" <<endl;
                    metadataStream << "AreaHeight="<<QString::number((double)img.rows/scale*1000000.0,'f')<< "\r" <<endl;
                    ...
                    metadataStream << "ZLengthPerDigitF=1"<< "\r"<<endl;
                    metadataStream << "[buffers]"<< "\r" <<endl;
                    metadataStream << "intensity="<<baseFileName<<"_intensity.bin" << "\r"<<endl;
                    metadataStream << "topography="<<baseFileName<<"_intensity.bin" << "\r"<<endl;
