...
bool MCompareVecH(const cv::Vec3f &a, const cv::Vec3f &b) {
    if (round(a[1]) / segment_size != round(b[1]) / segment_size)
        return a[1] < b[1];
    return a[0] < b[0];
}
bool MCompareVecV(const cv::Vec3f &a, const cv::Vec3f &b) {
    if (round(a[0]) / segment_size != round(b[0]) / segment_size)
        return a[0] < b[0];
    return a[1] < b[1];
}
            ...
            if ((j < input.rows) && (i < input.cols)) {
                ...
                if (temp == "0")
                    ...
                    if (((temp[0] - '0') > 0) && ((temp[0] - '0') <= 9)) {
                        ...
                        int k = 1;
                        while ((temp[k] != '\0') && (k < 100) && (((temp[k] - '0') <= 0) || ((temp[k] - '0') > 9)))
                            k++;
                        if (((temp[k] - '0') > 0) && ((temp[k] - '0') <= 9))
                            res[temp[k] - '0']++;
                    }
                }
            }
...
std::vector<double> evaluation::morphologicFeatures(cv::Mat mat, double scale) {
    ...
    double thresh = 0.9;
    double threshold =
        cv::threshold(gray, gray, (cv::mean(gray)[0] * thresh), 1, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
    ...
    if (circles.size() > 0) {
        ...
        for (int i = 0; i < dists.size(); i++) {
            if (dists.at(i) < dist_min)
                dist_min = dists.at(i);
            if (dists.at(i) > dist_max)
                dist_max = dists.at(i);
        ...
        for (size_t i = 0; i < circles.size(); i++) {
            ...
            if (round(circles[i][2]) / bucketsize <= 15)
                rpdf[round(circles[i][2]) / bucketsize] += 1.0 / circles.size();
            else
                rpdf[15] += 1.0 / circles.size();
        }
        ...
        for (size_t i = 0; i < circles.size() - 1; i++) {
            ...
            if ((abs(circles[i][1] - circles[i + 1][1]) < 150 * (scale / 500)) &&
                (abs(circles[i][0] - circles[i + 1][0]) < 300 * (scale / 500))) {
                validdists++;
                temp += abs(circles[i][0] - circles[i + 1][0]);
                if (round(abs(circles[i][0] - circles[i + 1][0]) / scale * 1000) / bucketsize <= 15)
                    hpdf[round(abs(circles[i][0] - circles[i + 1][0]) / scale * 1000) / bucketsize]++;
                else
                    hpdf[15]++;
            }
        }
        ...
        for (size_t i = 0; i < circles.size() - 1; i++) {
            ...
            if ((abs(circles[i][1] - circles[i + 1][1]) < 300 * (scale / 500)) &&
                (abs(circles[i][0] - circles[i + 1][0]) < 150 * (scale / 500))) {
                validdists++;
                temp += abs(circles[i][1] - circles[i + 1][1]);
                if (round(abs(circles[i][1] - circles[i + 1][1]) / scale * 1000) / bucketsize <= 15)
                    vpdf[round(abs(circles[i][1] - circles[i + 1][1]) / scale * 1000) / bucketsize]++;
                else
                    vpdf[15]++;
            }
        }
        ...
        for (int i = 0; i <= numbuckets; i++)
            vpdf[i] = vpdf[i] / validdists;
        temp = 0;
        bucketsize = 20 * scale / 1000; // bucketsize 20 micron
        for (int i = 0; i < dists.size(); i++) {
            temp += dists[i];
            if ((dists[i] / scale * 1000) / bucketsize <= 15)
                nnpdf[(dists[i] / scale * 1000) / bucketsize]++;
            else
                nnpdf[15]++;
        }
        nn_mean = temp / dists.size();
        for (int i = 0; i <= numbuckets; i++)
            nnpdf[i] = nnpdf[i] / dists.size();
    }

    for (int counter = 0; counter < cc.size(); counter++)
        morphologicFeat.push_back(cc[counter]);
    ...
    for (int counter = 1; counter < rpdf.size(); counter++)
        morphologicFeat.push_back(rpdf[counter]);
    for (int counter = 1; counter < hpdf.size(); counter++)
        morphologicFeat.push_back(hpdf[counter]);
    for (int counter = 1; counter < vpdf.size(); counter++)
        morphologicFeat.push_back(vpdf[counter]);
    for (int counter = 1; counter < nnpdf.size(); counter++)
        morphologicFeat.push_back(nnpdf[counter]);
            ...
            for (int i = 0; i < Blocksize; i++)
                for (int j = 0; j < Blocksize; j++)
                    if ((row + j < mat.rows) && (col + i < mat.cols)) {
                        if (gray.at<unsigned char>(row + j, col + i) > 0)
                            temp++;
                        num++;
                    }
            if (temp > ((double)num) * thresh)
                res++;
        }
    ...
    if (circles.size() > 0) {
        for (int i = 0; i < circles.size(); i++) {
            ...
            for (int j = 1; j < circles.size(); j++) {
                if (i != j) {
                    dist = sqrt(pow((double)(circles[i][0] - circles[j][0]), 2) +
                                pow((double)(circles[i][1] - circles[j][1]), 2));
                    if ((dist < mindist) && (dist > circles[i][2]) && (dist > circles[j][2]))
                        mindist = dist;
                }
            }
            distances[i] = mindist;
...
std::vector<double> evaluation::circleAnalyzer(std::vector<cv::Vec3f> circles, cv::Mat input) {
    ...
    if (circles.size() > 0) {
        for (int i = 0; i < circles.size(); i++) {
            temp = input.clone();
            area = cv::floodFill(temp, cv::Point(circles[i][0], circles[i][1]), cv::Scalar(128), &rect, cv::Scalar(1),
                                 cv::Scalar(1), 4);
