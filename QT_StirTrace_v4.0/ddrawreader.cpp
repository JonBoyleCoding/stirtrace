...
bool DDRawReader::readData(QString file) {
    ...
    for (int line=0; line<metadataLines.length(); line++) {
        ...
        if (lineSplit.at(0)=="ScaleX") this->scale=(lineSplit.at(1).toDouble()/100000.0);
        if (lineSplit.at(0)=="Height") height=(lineSplit.at(1).toInt());
        if (lineSplit.at(0)=="Width") width=(lineSplit.at(1).toInt());
        if (lineSplit.at(0)=="StartTime") this->time=lineSplit.at(1).toInt();
        if (lineSplit.at(0)=="intensity") intensityfile=lineSplit.at(1);
         if (lineSplit.at(0)=="topograpy") topographyfile=lineSplit.at(1);
         if (lineSplit.at(0)=="color") colorfile=lineSplit.at(1);
         if (lineSplit.at(0)=="colorintensity") colorintensityfile=lineSplit.at(1);
    }
...
cv::Mat DDRawReader::getImage(QString ImageType) {
    ...
    if (index>=0) return this->images.at(index);
    return cv::Mat();
}
